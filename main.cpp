...
//generates an integer between minNum and maxNum,
//where maxNum > minNum and maxNum > 20 and minNum < -20.
//If the number is between
//-20 and 20 it will generate a new number
{
    int randInt = 0;
    srand(time(NULL));
...
public:
    ...
    {
        //CHECK IF THE BALL HITS A PADDLE (first check the right paddle, then the left paddle)
...
{
    std::cout << "paddleLeft: " << paddleLeft.getScore() << " paddleRight: " << paddleRight.getScore() << std::endl;
}
...
{
    //CREATE WINDOW
    sf::RenderWindow window(sf::VideoMode(800,600), "Ashi Network Pong");
    ...
    //BALL (float posX, float posY, float speedX, float speedY, float radius=3)
    ...
    //REGISTER PADDLES TO INPUTMANAGER
    //inputManager.addObserver(&paddleRight);
    //inputManager.addObserver(&paddleLeft);

    std::cout << "Bist du der Server(s) oder der Client(c)? ";
    ...
    //CLOCK RESTART, CAN EVENTUALLY BE DELETED LATER
    clock.restart();
